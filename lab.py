...
seq_len = 2048
shift_size = 2048
category = np.arange(6)


def load_mat(data_dir):
    signal = scio.loadmat(data_dir)['Datas'][:, 1]
...
def get_train_test_from_files(data_dir, valid_ratio, test_ratio, partial_train):
    ...
    datasetname.sort(
        key=lambda tn: int(re.match('^category(\d+)', tn).group(1)))

    for i in range(len(category)):

        dir_zju = os.path.join(data_dir, datasetname[i], '6.mat')
        signal = load_mat(dir_zju)
        ...
        dir_zju = os.path.join(data_dir, datasetname[i], '7.mat')
        signal = load_mat(dir_zju)
        ...
        dir_zju = os.path.join(data_dir, datasetname[i], '8.mat')
        signal = load_mat(dir_zju)
        ...
        dir_zju = os.path.join(data_dir, datasetname[i], '9.mat')
        signal = load_mat(dir_zju)
        ...
        dir_zju = os.path.join(data_dir, datasetname[i], '10.mat')
        signal = load_mat(dir_zju)
...
class LAB(object):
    ...
    def data_prepare(
            self,
            valid_ratio=0.3, test_ratio=0.3,
            partial_train=1.,
